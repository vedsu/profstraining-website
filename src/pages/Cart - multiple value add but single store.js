...
const savePendingOrder = async () => {
    ...
    try {
        ...
        formData.append("topic", firstItem?.title || "");
        formData.append("webinardate", firstItem?.date || "");

        const live = cartItems.find(x => x.optionName === "Live Session");
        const recording = cartItems.find(x => x.optionName === "Recording");
        const digital = cartItems.find(x => x.optionName === "Digital Download");
        const transcript = cartItems.find(x => x.optionName === "Transcript PDF");

        formData.append("sessionLive", live ? "true" : "false");
        formData.append("priceLive", live ? live.price : 0);

        formData.append("sessionRecording", recording ? "true" : "false");
        formData.append("priceRecording", recording ? recording.price : 0);

        formData.append("sessionDigitalDownload", digital ? "true" : "false");
        formData.append("priceDigitalDownload", digital ? digital.price : 0);

        formData.append("sessionTranscript", transcript ? "true" : "false");
        formData.append("priceTranscript", transcript ? transcript.price : 0);
